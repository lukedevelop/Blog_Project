...
export default function BoardWriter() {
  ...
  const getBoardResponse = (responseBody: GetBoardResponseDto | ResponseDto | null) => {
    ...
    contentRef.current.style.height = `${contentRef.current.scrollHeight}px`; // 스크롤 없애고 자연스럽게
  }
  ...
  const onTitleChangeHandler = (event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    titleRef.current.style.height = `${titleRef.current.scrollHeight}px`; // 스크롤 없애고 자연스럽게

  }

  //          event handler: 내용 변경 이벤트 처리          //
  const OnContetnChangeHandler = (event: ChangeEvent<HTMLTextAreaElement>) => {
    const { value } = event.target;
    setContent(value);
  ...
  const OnImageChangeHandler = (event:ChangeEvent<HTMLInputElement>) => { 
    if(!event.target.files || !event.target.files.length) return // 파일길이가 0이거나 파일이 없으면 리턴 처리
    const file = event.target.files[0];
    
    //파일 이미지 미리보기
    const imageUrl = URL.createObjectURL(file); // 파일 url을 뽑아온다.
    ...
    imageInputRef.current.value = ''; // 똑같은 이미지를 넣기위해 빈값으로
  }
      ...
      <div className='board-update-container'>
        <div className='board-update-box'>
          ...
          <div className='board-update-content-box'>
            <textarea ref={contentRef} className="board-update-content-textarea" placeholder='본문을 작성해주세요.' value={content} onChange={OnContetnChangeHandler}/>
          ...
          <div className='board-update-images-box'>
            {imageUrls.map((imageUrl, index) =>  // 이미지 반복
