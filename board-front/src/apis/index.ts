...
const PATHC_BOARD_URL = (boardNumber: number | string) => `${API_DOMAIN}/board/${boardNumber}`; 
...
export const patchBoardRequest = async (boardNumber: number | string, requestBody: PatchBoardRequestDto, accessToken: string) => {
    const result = await axios.patch(PATHC_BOARD_URL(boardNumber), requestBody, authorization(accessToken))
        .then(response => {
...
const GET_RELATION_LIST_URL = (searchWord: string) => `${API_DOMAIN}/${searchWord}/relation-list`;
